...
(test ModM
  ...
  (is (equal '(:success) (match-all 15 (ModM 7) ((val 8) :success))))
  (is (equal '(:success) (match-all 15 (ModM 7) ((val 22) :success)))))
...
(test CardM
  ...
  (is (equal '(:success)
             (match-all '(card :club 12) (CardM)
               ((card (val :club) (val 12)) :success))))
  (is (equal '(:success)
             (match-all '((card :spade 11) (card :spade 12)) (ListM (CardM))
               ((cons (card suit num)
                      (cons (card (val suit) (val (+ 1 num)))
                             _))
                 :success))))
  (is (equal '(:success)
